...
func (ctm *Customer) SaveCustomer(db *sqlx.DB) error {
	var check_exist int

	sqlexist := `select count(code) as check_exist from dbo.bccustomer where code = ?`
	...
	switch {
	case errint == i.Scan(ctm.WorkAddress) && errint != nil:
		ctm.WorkAddress = ctm.BillAddress
	case errint == i.Scan(ctm.PersonType) && errint != nil:
		ctm.PersonType = 0
	}
